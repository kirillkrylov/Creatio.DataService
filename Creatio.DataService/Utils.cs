	...
	public class Utils : IDisposable
	{
		...
		private SelectQuery BuildSelectRequest(QueryParameters queryParameters, List<QueryColumn> queryColumns)
		{
			SelectQuery selectQuery = new SelectQuery()
			{
				...
				UseRecordDeactivation = queryParameters.UseRecordDeactivation,
		...
		/// <summary>
		/// Converts server response to DataTable with the datatype and column name inferred from the response definition
		/// </summary>
		...
		public async Task<bool> LoginAsync()
		{
			...
			if (Instance.IsLoginSuccess)
			{
				Instance.CurrentUser = await GetSysValuesAsync();
				cts = new CancellationTokenSource();
				Instance.ConectWebSocket(cts.Token);
			}
			return Instance.IsLoginSuccess;
		}
		...
		public async Task<List<Entity>> Select<Entity>(Filters filters, bool useLoc = false) where Entity : BaseEntity, new()
		{
			QueryParameters queryParameters = BuildQueryParameters<Entity>(useLoc);
			List<QueryColumn> queryColumns = BuildQueryColumns<Entity>();
			SelectQuery selectQuery = BuildSelectRequest(queryParameters, queryColumns);

			selectQuery.Filters = filters;
			string selectQueryJson = JsonConvert.SerializeObject(selectQuery);
			try
			{
